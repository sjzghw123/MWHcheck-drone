...
#update:bug修复=kafka巡检脚本无kafka-log-dirs.sh命令/kafka端口取值异常/kafka版本号取值异常/kafka巡检脚本security_baseline_dir取值异常/kafka巡检脚本安全基线取值异常
...
function collect_sys_info() {
    ...
    echo "----->>>---->>>  disk mount "
    df -h
    echo "----->>>---->>> zookeeper process info"    
    ps -ef | grep java |grep zookeeper | grep -v grep
...
function kafka_inquiry_info() {
    ...
    kafka_bin=$(pwdx $kafka_pid | awk -F ":" '{print $2}')
    kafka_dir=$(dirname $kafka_bin)
    ...
    cd $kafka_dir/libs
    Kafka_Version=`ls -al|grep kafka_|grep -v -e "javadoc" -e "scaladoc" -e "sources" -e "test" -e "asc"|awk -F " " '{print $NF}'|awk -F "-" '{print $NF}' | tr -cd "[0-9]"|awk '{print $1,$3}'`
    Kafka_Version=`echo ${Kafka_Version:0:3}`
    ...
    if [ $Kafka_Version -ge "220" ]; then
        ...
        echo ""
        echo "================查询消费者成员信息======================"
        #在kafka0.11.0的版本中，如下命令缺少--members 和--state的参数，所以如下两条命令  不做删除，因为有部分版本的kafka可以执行如下命令；
        for i in $kafka_consumer_group_list; do
...
function kafka_security_baseline() {
    ...
    for line in $(ps -ef | grep kafka.Kafka | grep -v grep | awk -F " " '{print $2}'); do
        ...
        security_baseline_dir=$(ls -lrt $kafka_dir | grep -v total |grep -v 总用量| head -n 1 | awk -F " " '{print $3}') && echo "security_baseline_dir:$security_baseline_dir"
