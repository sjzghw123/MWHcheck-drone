...
#update：整改zk获取port不准确、config显示不全、获取jvm参数异常bug
...
function zookeeper_inquiry_info() {
    ...
    if [ $zookeeper_pid_member == 0 ]; then
        ...
        ps -ef | grep java | grep zookeeper|grep quorum
...
function get_zookeeper_security_baseline() {
    ...
    Security_Baseline_Port=$Server_Port && echo "\"Security_Baseline_Port\"":"\"$Security_Baseline_Port\"""," >>$new_document
